...
export class StorageManager {
    ...
    try {
      ...
      if (cached) {
        return {
          ...
          attachments: Array.isArray(cached.attachments)
            ? cached.attachments
            : [],
      ...
      try {
        ...
        const attachments = Array.isArray(weekData.attachments)
    ...
    try {
      ...
      Object.keys(this._weeksCache).forEach((k) => {
        out[k] = {
          ...
          attachments: Array.isArray(this._weeksCache[k].attachments)
            ? this._weeksCache[k].attachments
            : [],
  ...
  static verifyFolderPassword(password) {
    try {
      const storedHash =
        this._configCache[CONFIG.FOLDER_PASSWORD_KEY] ||
        localStorage.getItem(CONFIG.FOLDER_PASSWORD_KEY);
  ...
  static getFolderData() {
    try {
      if (Array.isArray(this._foldersCache) && this._foldersCache.length > 0)
        return JSON.parse(JSON.stringify(this._foldersCache));
      // Fallback to legacy localStorage and migrate into cache
      try {
        const storedData = JSON.parse(
          localStorage.getItem(CONFIG.FOLDER_STORAGE_KEY) || "[]",
        );
  ...
  static deleteFolder(folderId) {
    try {
      const folders = this.getFolderData().filter(
  ...
  static getFolderById(folderId) {
    try {
      const found = (
        Array.isArray(this._foldersCache)
          ? this._foldersCache
          : this.getFolderData()
    ...
    try {
      ...
      if (data.weeks && typeof data.weeks === "object") {
        Object.keys(data.weeks).forEach((k) => {
          ...
          try {
            this._weeksCache[String(k)] = {
              highlights: wk.highlights || "",
              attachments: wk.attachments || [],
              important: !!wk.important,
            };
            this._put("weeks", {
              week: Number(k),
              highlights: wk.highlights || "",
  ...
  static isFolderSetupComplete() {
    try {
      if (this._configCache[CONFIG.FOLDER_SETUP_COMPLETE_KEY])
        return this._configCache[CONFIG.FOLDER_SETUP_COMPLETE_KEY] === "true";
      try {
  ...
  static markFolderSetupComplete() {
    try {
      ...
      this._put("config", {
        key: CONFIG.FOLDER_SETUP_COMPLETE_KEY,
        value: "true",
      });
      return true;
    } catch {
  ...
  static saveSecurityQuestions(question1Answer, question2Answer) {
    try {
      ...
      this._put("config", {
        key: CONFIG.FOLDER_SECURITY_QUESTIONS_KEY,
        value: questions,
      });
      return true;
    } catch {
  ...
  static verifySecurityQuestions(question1Answer, question2Answer) {
    try {
      const stored =
        this._configCache[CONFIG.FOLDER_SECURITY_QUESTIONS_KEY] ||
        JSON.parse(
          localStorage.getItem(CONFIG.FOLDER_SECURITY_QUESTIONS_KEY) || "{}",
        );
      const q1Matches =
        stored.q1 === this.hashPassword(question1Answer.toLowerCase().trim());
      const q2Matches =
        stored.q2 === this.hashPassword(question2Answer.toLowerCase().trim());
  ...
  static getPasswordAttempts() {
    try {
      const attemptVal =
        this._configCache[CONFIG.FOLDER_PASSWORD_ATTEMPTS_KEY] ||
        localStorage.getItem(CONFIG.FOLDER_PASSWORD_ATTEMPTS_KEY);
  ...
  static incrementPasswordAttempts() {
    try {
      ...
      this._put("config", {
        key: CONFIG.FOLDER_PASSWORD_ATTEMPTS_KEY,
        value: next,
      });
      try {
